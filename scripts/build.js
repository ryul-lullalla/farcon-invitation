      ...
      const { loadLocal } = await inquirer.prompt([
        {
          ...
          name: "loadLocal",
          message:
            "Found .env.local - would you like to load its values? (except for SEED_PHRASE, values will be written to .env)",
          default: false,
        },
      ]);
...
async function generateFarcasterMetadata(
  ...
  return {
    ...
    frame: {
      ...
      imageUrl: `https://${domain}/images/icon.png`,
      buttonTitle: process.env.NEXT_PUBLIC_FRAME_BUTTON_TEXT,
      splashImageUrl: `https://${domain}/splash_logo.png`,
      ...
      const { seedPhrase: inputSeedPhrase } = await inquirer.prompt([
        {
          ...
          name: "seedPhrase",
          message:
            "Enter your seed phrase (this will only be used to sign the frame manifest):",
          validate: async (input) => {
            try {
